                                ...
                                LIB_ERR     *perr)
{
    ...
    size_rem = Mem_PoolGetSizeRem(&Mem_PoolHeap, align, perr);
...
/*$PAGE*/
    ...
    for (i = 0u; i < blk_nbr; i++) {                                /* Alloc mem blks from mem seg.                     */
        pmem_blk = (void *)Mem_PoolSegAlloc(pmem_seg, blk_size, blk_align);
...
*                                        Mem_PoolGetSizeRem()
*
* Description : Get remaining memory pool size available to allocate.
*
...
*               perr        Pointer to variable that will receive the return error code from this function :
*
*                               LIB_MEM_ERR_NONE                Memory pool remaining size successfully 
*                                                                   returned.
*                               LIB_MEM_ERR_NULL_PTR            Argument 'pmem_pool' passed a NULL pointer.
*                               LIB_MEM_ERR_INVALID_POOL        Invalid memory pool type.
*                               LIB_MEM_ERR_INVALID_MEM_ALIGN   Invalid memory alignment.
*
* Return(s)   : Remaining memory pool size (in octets) [see Note #1], if NO error(s).
*
*               0,                                                    otherwise.
*
* Caller(s)   : Application.
*
* Note(s)     : (1) Remaining size of memory pool returned from either :
*
...
/*$PAGE*/
#if (LIB_MEM_CFG_ALLOC_EN == DEF_ENABLED)
CPU_SIZE_T  Mem_PoolGetSizeRem (MEM_POOL    *pmem_pool,
                                CPU_SIZE_T   align,
                                LIB_ERR     *perr)
{
    MEM_POOL    *pmem_seg;
